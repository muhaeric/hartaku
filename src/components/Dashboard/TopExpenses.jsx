 ...
 * colour per category. Identity is carried by the swatch and the always-visible
 * label; every bar is directly labelled with its value.
 */
export default function TopExpenses ({ breakdown, categories, limit = 5 }) {
  ...
  return (
    <ul className="space-y-2.5 px-page py-3">
      {rows.map((row) => {
        ...
        return (
          <li key={row.name} title={`${row.name}: ${share}% dari total pengeluaran`}>
            <div className="mb-1 flex items-baseline justify-between gap-3">
              ...
              <span className="shrink-0 text-body font-semibold amount">
                {formatCurrency(row.total, settings.currency)}
              </span>
            </div>

            <div className="h-1.5 w-full overflow-hidden rounded-full bg-hairline dark:bg-hairline-dark">
              <div
